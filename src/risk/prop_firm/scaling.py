...
def evaluate_scaling(
    ...
    if not executions:
        return ScalingReport(
            attempts=[], total_duration_days=0, active_attempt_index=-1
        )
    ...
    current_tier_type = (
        "CHALLENGE"
        if challenge_config.program_id.startswith("CTI_2STEP")
        else "INSTANT"
    )
    ...
    min_review_date = current_level_start + relativedelta(
    ...
    while i < len(sorted_execs):
        ...
        failed = False
        floor = (
            start_tier_equity - max_dd_amount
            if challenge_config.drawdown_type == "STATIC"
            else peak_balance - max_dd_amount
        )
        ...
        if not failed and challenge_config.max_daily_loss_pct:
            ...
            limit = day_start - (
                nominal_current_size * challenge_config.max_daily_loss_pct
            )
        ...
        if failed or promoted:
            ...
            if failed:
                ...
                if instant_cost_map:
                    affordable_instants = sorted(
                        [t for t, c in instant_cost_map.items() if c <= buyback_budget],
                        reverse=True,
                    )
                    if affordable_instants:
                        ...
                        affordable_challenges = sorted(
                            [t for t, c in cost_map.items() if c <= buyback_budget],
                            reverse=True,
                        )
                        if affordable_challenges:
                            ...
                            nominal_current_size = scaling_config.increments[0]
                            pending_buyback_cost = cost_map.get(
                                nominal_current_size, challenge_config.cost
                            )
                            buyback_type = "CHALLENGE"
                    else:
                ...
                try:
                    current_tier_idx = scaling_config.increments.index(
                        nominal_current_size
                    )
                except ValueError:
                    current_tier_idx = 0
    ...
    return ScalingReport(
        ...
        payout_share=challenge_config.payout_share,
    )
