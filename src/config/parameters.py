...
class StrategyParameters(BaseModel):
    ...
    def recovery_must_be_less_than_drawdown(cls, v, info):
        ...
        if (
            "max_drawdown_threshold" in info.data
            and v >= info.data["max_drawdown_threshold"]
        ):
    ...
    def high_vol_must_exceed_low_vol(cls, v, info):
        ...
        if (
            "low_volatility_percentile" in info.data
            and v <= info.data["low_volatility_percentile"]
        ):
            raise ValueError(
                "high_volatility_percentile must be greater than low_volatility_percentile"
            )
        return v
