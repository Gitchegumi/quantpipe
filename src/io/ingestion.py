        ...
        with PerformanceTimer() as timer:
            ...
            if df is None:
                ...
                if return_polars:
                    ...
                    df = pl.read_csv(path, has_header=True)  # Assuming header for Polars
                    ...
                    input_row_count = len(df)
                        ...
                        if df is None:
                            ...
                            logger.warning("No data loaded, proceeding with empty DataFrame.")
                        ...
                        if return_polars:
                            ...
                            if "timestamp" in df.columns:
                                ...
                                df = df.with_columns(
                                    ...
                                    pl.col("timestamp").str.to_datetime(time_unit="ns").alias("timestamp_utc")
            ...
            if len(df) == 0:
                ...
                if return_polars:
                    ...
                    df = df.with_columns(pl.Series("is_gap", [False] * len(df), dtype=pl.Boolean))
                ...
                core_hash = compute_dataframe_hash(df, CORE_COLUMNS, is_polars=return_polars)
                ...
                if fill_gaps:
                    ...
                    gap_indices = detect_gaps(df, timeframe_minutes, is_polars=return_polars)
                    ...
                    if gaps_detected > 0:
                        ...
                        df, gaps_inserted = fill_gaps_vectorized(df, timeframe_minutes, is_polars=return_polars)
                ...
                if downcast and not return_polars: # Downcasting only for Pandas for now
