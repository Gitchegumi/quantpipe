...
from src.io import cadence, duplicates, gaps, gap_fill, downcast, progress
from src.io import schema, timezone_validate, hash_utils
...
class TestCadence:
    ...
    def test_validate_cadence_uniformity_fail(self):
        ...
        df = pd.DataFrame(
            {
                "timestamp_utc": pd.date_range(
                    "2024-01-01", periods=90, freq="1min", tz="UTC"
                ),
                "close": np.arange(90),
            }
        )
        ...
        with pytest.raises(
            RuntimeError, match="Cadence deviation exceeds tolerance"
        ):
...
class TestDuplicates:
    ...
    def test_remove_duplicates(self):
        ...
        ts_filter = cleaned["timestamp_utc"] == pd.Timestamp(
            "2024-01-02", tz="UTC"
        )
...
class TestGapFill:
    ...
    def test_synthesize_gap_values(self):
        ...
        assert filled.loc[1, "is_gap"] is True
...
class TestProgress:
    ...
    def test_progress_reporter_limits_stages(self):
        """Test progress reporter enforces stage limit."""
        reporter = progress.ProgressReporter(total_stages=10)
        assert reporter.total_stages == 5  # Capped to MAX_PROGRESS_UPDATES

    def test_progress_report_stage(self):
        """Test stage reporting."""
        reporter = progress.ProgressReporter(total_stages=3)
        reporter.report_stage(IngestionStage.READ)
        assert reporter.current_stage == 1
        assert not reporter.is_complete()
